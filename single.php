...
<div class="flex flex-col">
	<?php while (have_posts()) : the_post(); ?>
		...
		<div class="relative mx-auto md:order-2 md:mt-4 xl:container xl:px-5">
			...
			<a href="javascript:window.history.back();" class="absolute top-0 left-0 flex items-center pt-6 pl-5 text-white md:hidden">
			...
			<?php the_post_thumbnail('large', array('class' => 'object-cover w-full mx-auto max-w-5xl shadow-lg')); ?>

		</div>
		<div class="px-5 mx-auto md:max-w-3xl md:order-1">
			<h1 class="mt-4 text-xl font-bold leading-tight text-gray-700 sm:text-3xl md:text-4xl md:mt-12"><?php the_title() ?></h1>
			<div class="block mt-3 text-gray-700"><?php the_date() ?> in <span class="inline-block font-bold"><?php the_category() ?></span></div>
		</div>
		<div class="max-w-3xl px-5 mx-auto mt-4 prose md:order-3">
			<?php the_content() ?>
		</div>
